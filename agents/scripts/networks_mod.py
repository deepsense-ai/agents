...
class Distribution(ABC):
    ...
    @abstractmethod
    def max_likelihood_op(self):
        pass

class MultivariateNormalDiag(Distribution):
    """
    Wraps tf.contrib.distributions.MultivariateNormalDiag with methods used by PPO
    """

    def __init__(self, mean, logstd):
        self._mean = mean
        self._logstd = logstd
        self.dist = tf.contrib.distributions.MultivariateNormalDiag(mean, tf.exp(logstd))

    def sample_op(self):
        return self.dist.sample

    def max_likelihood_op(self):
        return lambda:self._mean




NetworkOutput = collections.namedtuple(
    'NetworkOutput', 'policy, distribution_params, value, state')
...
def feed_forward_gaussian(
  ...
  policy = MultivariateNormalDiag(mean, tf.exp(logstd))
  distribution_params = tf.concat([mean, logstd], axis=0)
  # distribution_params = mean
  return NetworkOutput(policy, distribution_params, value, state)
